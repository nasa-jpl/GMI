...
#----------------------------------------------------------------------------
            ...
            FC='ifort -fp-stack-check -C -traceback -fstack-protector' ## -check all '
	    FFLAGS="-nologo -fpic -fpp -132 -gen-interfaces -fp-model strict -fno-omit-frame-pointer"
	    FFLAGS="$FFLAGS -D__amd64 -module $SMACOS_OBJS"

	    FLIBS="$FLIBS $RPATH $MLIBS -lm -lstdc++ "       # Matlab libs
	    FLIBS="$FLIBS $SMACOS_LIBS "                     # SMACOS libs -- ordering important

            FOPTIMFLAGS='-O2 -xHOST'
            FDEBUGFLAGS='-g'

            LD="$COMPILER"
            LDEXTENSION='.mexa64'
            LDFLAGS="-shared-intel -shared -Wl,--version-script,$TMW_ROOT/extern/lib/$Arch/$MAPFILE"
...
CFLAGS="$CFLAGS -DGMI_SVN_REV=\"'$GMI_SVN_REV'\" -DGMI_DATE=\"'$GMI_DATE'\""
FFLAGS="$FFLAGS -DGMI_SVN_REV=\"'$GMI_SVN_REV'\" -DGMI_DATE=\"'$GMI_DATE'\""
